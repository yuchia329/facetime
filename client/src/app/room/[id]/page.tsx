...
export default function RoomPage({ params }: PageProps) {
  ...
  useEffect(() => {
    // Handle dynamic viewport sizing for pagination and layout
    const handleResize = () => {
  ...
  // If speaker changes, switch to speaker mode if in grid for emphasis (optional)
  // Or just let user manually select mode. Let's make it manual toggle, but active speaker highlights.
  ...
  // In speaker mode, we lock onto the pinned user if they exist
  if (layoutMode === 'speaker') {
    if (pinnedUserId) {
      if (pinnedUserId === 'local') {
        speakerStream = localStream;
        speakerName = displayName;
        speakerIsLocal = true;
      } else {
        const pinnedPeer = participants.find((p) => p.peerId === pinnedUserId);
        if (pinnedPeer) {
          speakerStream = pinnedPeer.stream;
          speakerName = pinnedPeer.displayName;
          speakerIsLocal = false;
        } else {
          // Fallback if pinned user left
          if (dominantSpeakerId !== 'local' && dominantSpeakerId !== null && participants.length > 0) {
            const remoteActive = participants.find((p) => p.peerId === dominantSpeakerId) || participants[0];
            speakerStream = remoteActive.stream;
            speakerName = remoteActive.displayName;
            speakerIsLocal = false;
          }
        }
      }
  ...
  // Build the grid list explicitly for paginating remote users
  ...
  // Bandwidth Optimization: Tell the SFU which remote peers are physically visible on screen
  const visiblePeerIds = new Set<string>();
  if (layoutMode === 'speaker') {
    if (!speakerIsLocal && speakerStream) {
      const p = participants.find(part => part.stream === speakerStream);
      if (p) visiblePeerIds.add(p.peerId);
    }
    if (showFilmstrip) {
      displayFilmstrip.forEach(p => visiblePeerIds.add(p.peerId));
    }
  } else {
    displayGridPeers.forEach(p => {
      if (!p.isLocal) visiblePeerIds.add(p.peerId);
    });
  }
  const visiblePeerStr = Array.from(visiblePeerIds).sort().join(',');
  ...
  return (
    <div className={`room-page ${isChatOpen ? 'room-page--with-chat' : ''} ${isPortrait ? 'env-portrait' : 'env-landscape'}`}>
      ...
      <Controls
        ...
        onLeave={handleLeave}
        // New layout/chat props passed to Controls
